<script lang="ts">
  ...
  const slots = $derived.by(() =>
    normalizeVariantResult(alertDialogVariants({status}), "alert-dialog"),
  );
  const slotClass = $derived(slots.icon?.({}) ?? "alert-dialog__icon");
</script>
...
>
  ...
  {:else if status === "default" || status === "accent"}
    <svg
      aria-hidden="true"
      data-slot="alert-dialog-default-icon"
      fill="none"
